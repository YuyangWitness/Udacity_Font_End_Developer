...
var Engine = (function(global) {
    ...
    function main() {
         ...
         * 可以使动画更加顺畅。
         */
        ctx.clearRect(0,0,505,50);  //清除人物到最顶端的痕迹
        update(dt);
        render();
    ...
    function checkCollisions() {
        ...
        //如果其中有一个敌人与之碰撞就进入重新开始游戏页面
        allEnemies.forEach( enemy => {
            
            if(playerX - enemy.x < 90 && playerX - enemy.x > -90 && playerY - enemy.y <= -5 && playerY - enemy.y >= -15 ){
                // 游戏重新开始
                reset();                
            }
        } );

        //如果与金币碰撞，那么增加分数并且金币消失
        allGoods.forEach( (goods,index) => {
             if(goods.x === playerX && goods.y - playerY === 55 ){
                // 游戏重新开始
                allGoods.splice(index,1);
                addScore(goods.score);
            }
        })
    ...
    function winGame(){
        if(player.y === -25 && allGoods.length === 0){
            next();
        }
    } 
     ...
     * 重新开始游戏的按钮，也可以是一个游戏结束的画面，或者其它类似的设计。它只会被 init()
     * 函数调用一次。
     */
    function reset() {
        begin = false;
        ...
        let mask = document.querySelector("#mask");
        let FinallScore = document.querySelector(".final_score");
        FinallScore.textContent = allScore;     
        hideAndShow(restartDiv);
        hideAndShow(mask);
    }
