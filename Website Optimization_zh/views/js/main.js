...
var resizePizzas = function(size) {
  ...
  //提取节点数组并且获取其length，防止重复获取浪费时间
  var randomPizzaContainers = document.querySelectorAll(".randomPizzaContainer");
  var randomPizzaContainersLenth = randomPizzaContainers.length;

  function getRandomPizzaContainerOffWidth() {
    var rpcowArry = [];
	  for (var i = 0; i < randomPizzaContainersLenth; i++) {
		  var res = randomPizzaContainers[i].offsetWidth;
		  rpcowArry.push(res);
	  }
  ...
  function changePizzaSizes(size) {
    ...
    var rpcowArry = getRandomPizzaContainerOffWidth();
    for (var i = 0; i < randomPizzaContainersLenth; i++) {

      var nowRandomPizzaContainer = randomPizzaContainers[i];

      var oldWidth = rpcowArry[i];
      ...
      var newwidth = (oldWidth + dx) + 'px';
		nowRandomPizzaContainer.style.width = newwidth;
    }
  }
...
window.addEventListener('scroll', function(){
  ...
  //使用requestAnimationFrame来获取运行这个函数，可以使得动画更加流畅
